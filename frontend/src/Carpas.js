...
function Carpas() {
  ...
  const handleGuardar = async (carpa) => {
    try {
      ...
      if (String(carpaOriginalEdicion.referencia) !== String(carpa.referencia)) {
        cambios.push(`Referencia: ${carpaOriginalEdicion.referencia} → ${carpa.referencia}`);
      }
      if (String(carpaOriginalEdicion.marca) !== String(carpa.marca)) {
        cambios.push(`Marca: ${carpaOriginalEdicion.marca} → ${carpa.marca}`);
      }
      if (String(carpaOriginalEdicion.proveedor || "") !== String(carpa.proveedor || "")) {
        cambios.push(`Proveedor: ${carpaOriginalEdicion.proveedor || "vacío"} → ${carpa.proveedor || "vacío"}`);
      }
      if (Number(carpaOriginalEdicion.costo) !== Number(carpa.costo)) {
        cambios.push(`Costo: $${Number(carpaOriginalEdicion.costo).toLocaleString("es-CO")} → $${Number(carpa.costo).toLocaleString("es-CO")}`);
      }
      if (Number(carpaOriginalEdicion.precio) !== Number(carpa.precio)) {
        cambios.push(`Precio: $${Number(carpaOriginalEdicion.precio).toLocaleString("es-CO")} → $${Number(carpa.precio).toLocaleString("es-CO")}`);
      }
                      ...
                      >
                        ...
                        {modoEdicion === c.id ? (
                          <>
                            <td className="p-2">
                              <input
                                value={c.referencia}
                                onChange={(e) =>
                                  actualizarCampo(c.id, "referencia", e.target.value)
                                }
                            ...
                            <td className="p-2">
                              <input
                                value={c.proveedor}
                                onChange={(e) =>
                                  actualizarCampo(c.id, "proveedor", e.target.value)
                                }
                                className="w-full border-2 border-blue-300 rounded-lg text-sm p-2 focus:ring-2 focus:ring-blue-500 outline-none"
                              />
                            </td>
                            ...
                            <td className="p-2">
                              <input
                                ...
                                value={c.precio}
                                onChange={(e) =>
                                  actualizarCampo(c.id, "precio", e.target.value)
                                }
                                className="w-full border-2 border-blue-300 rounded-lg text-sm p-2 focus:ring-2 focus:ring-blue-500 outline-none"
                              />
                            </td>
                          ...
                          <>
                            ...
                            <td className="p-3 text-gray-700">{c.proveedor || "—"}</td>
                            ...
                            <td className={`p-3 text-center font-semibold ${c.stock === 0 ? "text-red-600" : "text-gray-700"}`}>
                              {c.stock === 0 ? (
