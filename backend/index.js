...
const express = require('express');
const multer = require('multer');
const { extraerDatosPDF } = require('./utils/pdfParser');
...
// Supabase client (asegúrate de tener tus credenciales)
...
// ============================================
app.post('/api/procesar-lista-llantar', upload.single('pdf'), async (req, res) => {
  try {
    ...
    const datosLlantar = await extraerDatosPDF(req.file.buffer);
    ...
    for (const itemLlantar of datosLlantar) {
      // Buscar coincidencia en inventario por marca + medida (+ referencia si está disponible)
      const llantaDB = inventario.find(l => {
        ...
        // Si la llanta en DB tiene referencia completa, intentar match exacto
      ...
      if (!llantaDB) {
        continue; // No está en nuestro inventario, saltar
      }
      ...
      const margenDisponible = itemLlantar.precio - precioEsperado;
      const porcentajeMargen = (margenDisponible / precioEsperado) * 100;

      // Calcular margen REAL (precio público - costo empresa)
      ...
      const { error: errorUpdate } = await supabase
        .from('llantas')
        .update({
          precio_cliente: itemLlantar.precio,
          alerta_margen: alertaMargen,
          fecha_ultima_actualizacion: new Date().toISOString()
        })
        .eq('id', llantaDB.id);

      if (errorUpdate) {
        console.error(`❌ Error actualizando llanta ${llantaDB.id}:`, errorUpdate);
        continue;
      }
      ...
      resultado.detalles.push({
        ...
        razon: estadoActualizacion === 'critico' 
          ? `Precio público ($${itemLlantar.precio.toLocaleString()}) menor que costo esperado ($${Math.round(precioEsperado).toLocaleString()})`
          : null
      });
    }
    ...
    console.log(`   - Margen Bajo: ${resultado.margenBajo}`);
    console.log(`   - Bloqueadas: ${resultado.bloqueadas}`);

    res.json(resultado);

  } catch (error) {
    console.error('❌ Error en /api/procesar-lista-llantar:', error);
    res.status(500).json({ 
