...
function App() {
  ...
  const actualizarCampo = (id, campo, valor) => {
    setItemEditado({ ...itemEditado, [campo]: valor });
  };
    ...
    <div className="max-w-7xl mx-auto p-4">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => { localStorage.removeItem('acceso'); window.location.href = '/login'; }}
      ...
      {mensaje && <div className="text-green-600 mb-4 text-sm">{mensaje}</div>}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white p-4 rounded shadow-md border md:col-span-1">
          ...
          <button onClick={() => { setBusqueda(''); setMarcaSeleccionada(''); setAncho(''); setPerfil(''); setRin(''); }} className="w-full mt-2 bg-gray-200 hover:bg-gray-300 text-sm text-black py-1 rounded">
            Limpiar filtros
          </button>
        </div>
              ...
              <tbody>
                {filtradas.map((ll, index) => (
                  <tr key={ll.id} className="border-t text-center">
                    {editando === ll.id ? (
                      <>
                        <td className="p-2"><input value={itemEditado.referencia} onChange={e => actualizarCampo(ll.id, 'referencia', e.target.value)} className="border px-1" /></td>
                        <td className="p-2"><input value={itemEditado.marca} onChange={e => actualizarCampo(ll.id, 'marca', e.target.value)} className="border px-1" /></td>
                        <td className="p-2"><input value={itemEditado.proveedor} onChange={e => actualizarCampo(ll.id, 'proveedor', e.target.value)} className="border px-1" /></td>
                        <td className="p-2"><input type="number" value={itemEditado.costo_empresa} onChange={e => actualizarCampo(ll.id, 'costo_empresa', e.target.value)} className="border px-1 w-20" /></td>
                        <td className="p-2"><input type="number" value={itemEditado.precio_cliente} onChange={e => actualizarCampo(ll.id, 'precio_cliente', e.target.value)} className="border px-1 w-20" /></td>
                        <td className="p-2"><input type="number" value={itemEditado.stock} onChange={e => actualizarCampo(ll.id, 'stock', e.target.value)} className="border px-1 w-20" /></td>
                        <td className="p-2">
                          <button
                            onClick={async () => {
                              try {
                                await axios.post('https://mi-app-llantas.onrender.com/api/editar-item', itemEditado);
                                setMensaje('Guardado ✅');
                                const nuevas = [...llantas];
                                nuevas[index] = itemEditado;
                                setLlantas(nuevas);
                                setEditando(null);
                              } catch (err) {
                            ...
                            className="bg-green-600 text-white px-2 py-1 text-xs rounded mr-1"
                          >Guardar</button>
                          <button onClick={() => setEditando(null)} className="bg-gray-400 text-white px-2 py-1 text-xs rounded">Cancelar</button>
                        </td>
                      </>
                    ) : (
                      <>
                        ...
                        <td className={`p-2 ${ll.stock === 0 ? 'text-red-600' : ''}`}>{ll.stock}</td>
                        <td className="p-2">
                          <button
                            onClick={() => {
                              setEditando(ll.id);
